...
import { Home, Search, Plus, User, Navigation } from 'lucide-react';
...
export function BottomNav() {
  ...
  const items = [
    ...
    { to: '/navigation', icon: <Navigation className="w-5 h-5" />, label: 'Naviguer',  active: path === '/navigation' },
    {
      to: user ? '/profil' : '/auth',
          ...
          {items.map(item => (
            <Link key={item.to} to={item.to}
              className={`flex flex-col items-center gap-0.5 px-2 py-1.5 rounded-xl transition-colors ${
                item.isPrimary
                  ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-200 -mt-4 px-4 py-2.5'
                  : item.active
                  ? 'text-indigo-600'
                  : 'text-gray-400'
              }`}>
